...
///
/// Extracts key=value from Lua tables: `table = { key = value }` and
/// bare `key = "value"` patterns.
pub fn parseLua(allocator: Allocator, data: []const u8) !ParsedConfig {
    ...
    while (line_iter.next()) |raw_line| {
        ...
        if (std.mem.indexOf(u8, line, "= {") orelse std.mem.indexOf(u8, line, "={")) |eq_brace| {
            const table_name = std.mem.trim(u8, line[0..eq_brace], " \t");
            // Skip "local " prefix
            const clean_name = if (std.mem.startsWith(u8, table_name, "local "))
                std.mem.trim(u8, table_name[6..], " \t")
            else
                table_name;
        ...
        // Key = value inside a table or at top level
        if (std.mem.indexOfScalar(u8, line, '=')) |eq_pos| {
            // Skip lines that are comparisons (== ~=)
            if (eq_pos + 1 < line.len and line[eq_pos + 1] == '=') continue;
            if (eq_pos > 0 and line[eq_pos - 1] == '~') continue;

            var raw_key = std.mem.trim(u8, line[0..eq_pos], " \t");
            var raw_val = std.mem.trim(u8, line[eq_pos + 1 ..], " \t,");

            // Strip "local " from key
            ...
            var full_key_buf: [512]u8 = undefined;
            const full_key = if (in_table and current_table.len > 0)
                std.fmt.bufPrint(&full_key_buf, "{s}.{s}", .{ current_table, raw_key }) catch raw_key
            else
                raw_key;
