...
GSA can attach to _any_ game server, fingerprint it via protocol
...
*New in this version:* User-configurable server connections with support
for default servers and favorites list. See Configuration for details.
...
|Server Browser |inspector |Discover/fingerprint servers, add to
management
...
|Cross-Server Search |database |VQL full-text + vector search across
configs
|===
...
The `+gsa+` binary is built automatically by `+zig+` `+build+` and lives
at `+src/interface/ffi/zig-out/bin/gsa+`.
...
|`+gsa+` `+probe+` `+<host>+` `+[port]+` |Fingerprint a game server
(Steam, Minecraft, RCON, HTTP, TCP banner)

|`+gsa+` `+profiles+` |JSON listing of all supported games with
engine/port/config details
...
Then run `+just+` `+generate-clades+` — the cladogram updates
automatically.
