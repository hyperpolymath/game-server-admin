...
`.machine_readable/descriptiles/NEUROSYM.a2ml` still cites the stale SHA and should be
...
* `.machine_readable/descriptiles/` — `STATE.a2ml` (status, blockers, session history), `META`, `ECOSYSTEM`, `AGENTIC`, `NEUROSYM` (Hypatia config + `.hypatia-ignore` scope), `PLAYBOOK`.
