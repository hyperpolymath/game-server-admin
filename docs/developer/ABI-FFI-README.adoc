...
# GameServerAdmin ABI/FFI Documentation
...
│  generated/abi/game-server-admin.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libgame-server-admin.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── game-server-admin.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── game-server-admin.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/game-server-admin.h
```
...
```c
#include "game-server-admin.h"

int main() {
    void* handle = game-server-admin_init();
    if (!handle) return 1;

    int result = game-server-admin_process(handle, 42);
    if (result != 0) {
        const char* err = game-server-admin_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    game-server-admin_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -lgame-server-admin -L./zig-out/lib
```

### From Idris2

```idris
import GameServerAdmin.ABI.Foreign
...
```rust
#[link(name = "game-server-admin")]
extern "C" {
    fn game-server-admin_init() -> *mut std::ffi::c_void;
    fn game-server-admin_free(handle: *mut std::ffi::c_void);
    fn game-server-admin_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = game-server-admin_init();
        assert!(!handle.is_null());

        let result = game-server-admin_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libgame-server-admin = "libgame-server-admin"

function init()
    handle = ccall((:game-server-admin_init, libgame-server-admin), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:game-server-admin_process, libgame-server-admin), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:game-server-admin_free, libgame-server-admin), Cvoid, (Ptr{Cvoid},), handle)
end
